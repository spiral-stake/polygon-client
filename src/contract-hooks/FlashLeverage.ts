import type { Token, CollateralToken, InternalSwapData, LeveragePosition } from "../types/index.ts";
...
export default class FlashLeverage extends Base {
      ...
      instance.collateralTokens = await Promise.all(
        _collateralTokens.map(
          async (collateralToken: CollateralToken): Promise<CollateralToken> => {
            const [collateralTokenValueInLoanToken, maxLtv, liqLtv, marketData, loanTokenPrice] =
              await Promise.all([
            ...
            return {
              ...
              loanToken: {
                ...
                valueInUsd: loanTokenPrice,
              },
              valueInLoanToken: collateralTokenValueInLoanToken,
              impliedApy: collateralToken.info.apy,
              valueInUsd: BigNumber(collateralTokenValueInLoanToken).multipliedBy(loanTokenPrice),
              borrowApy: marketData.borrowApy,
              liquidityAssets: BigNumber(marketData.liquidityAssets),
              safeLtv: maxLtv.toString(),
  ...
  async deleverage(leveragePosition: LeveragePosition, internalSwapData: InternalSwapData) {
    const txReceipt = await this.write("deleverage", [
      leveragePosition.id,
      internalSwapData.swapData,
    ]);
    ...
    const positions = await Promise.all(
      _userLeveragePositions.map(async (pos, index) => {
        ...
        const [amountLoan, { collateral: morphoCollateral }] = await Promise.all([
          this.calcUnleverageFlashLoan(collateralToken, pos.sharesBorrowed),
        ...
        return {
          ...
          amountLoan,
          amountLoanInUsd: amountLoan.multipliedBy(collateralToken.loanToken.valueInUsd),
          ...
          amountDepositedInUsd: amountCollateral.multipliedBy(collateralToken.valueInUsd),
