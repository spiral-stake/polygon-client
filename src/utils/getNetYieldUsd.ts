...
export const getNetYieldUsd = (
  ...
  const grossYield = (Number(leverageApy) / 100) * amountInUsd * (maturityDaysLeft / 365);
  const slippage = addSlippage ? amountInUsd * Number(leverage) * 0.001 : 0; // 0.1% slippage on amount leveraged
