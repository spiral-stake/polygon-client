...
import { getInternalSwapData } from "../api-services/swapAggregator.ts";
      ...
      <div className="flex flex-col lg:flex-row w-full gap-[48px]">
        <div className="flex gap-[8px] lg:flex-col lg:gap-[16px] overflow-x-scroll no-scrollbar">
          ...
          <RiskTab
            risk="medium"
            icon={mediumRisk}
            text="Token backed by standard DeFi mechanisms like Layer2 staking & funding rates"
            setRisk={setRisk}
            currentRisk={risk}
            color="rgba(242, 217, 103, 0.1)"
          />
          <RiskTab
            risk="low"
            icon={lowRisk}
            text="Tokens backed by highly credible collateral such as Staked ETH and T-bills"
            setRisk={setRisk}
            currentRisk={risk}
            ...
            {flashLeverage.collateralTokens
              ...
              .sort((a, b) =>
                sortMethod == "APY"
                  ...
                  : Number(
                    BigNumber(b.liquidityAssets)
                      .dividedBy(BigNumber(calcLeverage(b.maxLtv)).minus(1))
                      .minus(10)
                  ) -
                  Number(
                    BigNumber(a.liquidityAssets)
